    ...
    for i in range(num_tests):
        ...
        if should_fail:
            if i % 2 == 0:
                ...
                failure.set("message", "Assertion failed: Expected value was incorrect")
                failure.set("type", "AssertionError")
                failure.text = (
                    f"AssertionError: Expected 'success' but got 'failure' at line {i * 10 + 42}"
                )
                failures += 1
            else:
                # Add error
                error = ET.SubElement(testcase, "error")
                error.set("message", "Test runtime error")
                error.set("type", "RuntimeError")
                error.text = "RuntimeError: Connection timeout after 30 seconds"
                errors += 1
        else:
